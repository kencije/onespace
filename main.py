...
def draw_boss_health(health):
    ...
    pygame.draw.rect(skärm, (255, 0, 0), (bar_x, bar_y, bar_width, bar_height))
    pygame.draw.rect(skärm, (0, 255, 0), (bar_x, bar_y, bar_width * health_ratio, bar_height))
...
while spelet_körs:
    ...
    if boss_active:
        ...
        if critical_rect is None and random.random() < 0.001:
            spawn_critical()
        ...
        if critical_rect:
            ...
            if sprite_spelare.get_rect(topleft=(spelare_x, spelare_y)).colliderect(critical_rect):
                activate_critical()
                critical_rect = None

        if critical_active and pygame.time.get_ticks() - critical_timer > 5000:
            critical_active = False
